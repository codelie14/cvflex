...
const ExportModal = ({ cvData, theme, font, onClose, onImportJSON }) => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      toast({ title: "🎉 Export réussi !", description: "Votre CV a été téléchargé en PDF.", duration: 3000 });
    } catch (error) {
      console.error('Erreur PDF:', error);
      toast({ title: "❌ Erreur d'export", description: "Erreur lors de l'export PDF.", duration: 3000 });
    } finally {
  ...
  const exportToPNG = async () => {
    ...
    try {
      ...
      toast({ title: "🎉 Export réussi !", description: "Votre CV a été téléchargé en PNG.", duration: 3000 });
    } catch (error) {
      console.error('Erreur PNG:', error);
      toast({ title: "❌ Erreur d'export", description: "Erreur lors de l'export PNG.", duration: 3000 });
    } finally {
      setIsExporting(false);
    }
  };
  ...
  const exportToJSON = () => {
    ...
    toast({ title: "💾 Données exportées !", description: "Vos données CV ont été sauvegardées en JSON.", duration: 3000 });
  };
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          <p className="text-xs text-gray-400">Vos données sont automatiquement sauvegardées localement.</p>
        </div>
      </motion.div>
    </motion.div>
  );
